...
function CreatePostComp({ prop, toggle, settoggle,toolbar }: any) {
      ...
      <div id={!toolbar ? 'MainPageFade2' : ''} className={` items-end justify-between flex-col  h-full gap-2 lg:w-[80%] w-full ${!toolbar == false ? 'hidden' : 'flex' } `}>
        ...
        <textarea placeholder="Post Something ..." value={PostText}
          ...
          className={`${BorderToggler ? 'border-2 border-green-600' : ''} 
            transition-all  duration-300 ease-in-out font-light text-sm backdrop-blur-md bg-[#ede8e80a] h-[100%] lg:h-[90%] 
            w-full p-6 rounded-md md:rounded-sm text-left border-2 border-black focus:border-2 focus:border-pink-600/50`}>
        </textarea>
